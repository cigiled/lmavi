...
void read_rbsp_trailing_bits( bs_t* b)
{
    ...
    {
        rbsp_alignment_zero_bit = bs_read_u1( b ); // equal to 0
    }
	//printf("rbsp_stop_one_bit:[%d],rbsp_alignment_zero_bit:[%d]\n", rbsp_stop_one_bit, rbsp_alignment_zero_bit);
}
...
int spare_sps_pri(uint8_t *date, int sz)
{
	int m_nWidth;  
    int m_nHeight;  
...
int test_main()
{
	...
	#if  1
		//848-480.264:       848*480
		uint8_t date_s[] = { 0x42, 0x00, 0x1F, 0x8D, 0x68, 0x0D, 0x43, 0xDA, 0x10,
									0x00, 0x00, 0x03, 0x00, 0x10, 0x00, 0x00, 0x03, 0x01, 0xE0, 0xF1, 0x07, 0xA8};			  
		spare_sps_pri(date_s,  sizeof(date_s));

		//src.h264:      720*480 
		uint8_t date_s1[] = {0x4D, 0x00, 0x1E, 0x8D, 0x94, 0xC0, 0x5A, 0x1E, 0x88};			  
		spare_sps_pri(date_s1,  sizeof(date_s1));
		//nat.h264       704*576
		uint8_t date_s2[] = {0x4D, 0x40, 0x1E, 0x95, 0x15, 0x18, 0x80, 0x58, 0x09, 0x32};			  
		spare_sps_pri(date_s2,  sizeof(date_s2));

		//720P25.h264
		uint8_t date_s3[] = {0x64, 0x00, 0x1F, 0xAD, 0x84, 0x01, 0x0C, 0x20, 0x08, 0x61, 0x00, 0x43, 0x08, 0x02, 0x18, 0x40,
					  0x10, 0xC2, 0x00, 0x84, 0x2B, 0x50, 0x28, 0x02, 0xDD, 0x37, 0x01, 0x01, 0x01, 0x40, 0x00, 0x00,
					  0xFA, 0x00, 0x00, 0x3A, 0x98, 0x21};
		spare_sps_pri(date_s3,  sizeof(date_s3));
